...
fn analyze_line(
    ...
    match language {
        "Python" => {
            ...
            // Indentation is the only reliable scope signal available to this
            // line-based heuristic. Do not accumulate it across unrelated loops.
            state.loop_depth = state.nesting_depth;
        }
        "Go" => {
            ...
            state.nesting_depth = state
                .nesting_depth
                .saturating_add(opens)
                .saturating_sub(closes);

            // Brace depth is an approximation, but unlike a monotonically
            // increasing loop counter it returns to zero when blocks close.
            state.loop_depth = state.nesting_depth;
        }
        _ => {}
    }
    ...
    // ── Apply each rule ───────────────────────────────────────────────────────
    for rule in rules {
        // A capitalized call is not recursion. Only report it if the currently
        // scanned function invokes itself.
        if rule.vulnerability == VulnerabilityType::RecursiveCall {
            let self_call = Regex::new(&format!(
                r"\b{}\s*\(",
                regex::escape(&state.current_function)
            ))
            .expect("escaped function name must form a valid regex");
            if !self_call.is_match(raw_line) {
                continue;
            }
        }
